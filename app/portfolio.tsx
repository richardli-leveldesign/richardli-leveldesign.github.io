...
function KillTheMakersPage({ project }: { project: (typeof projects)[number] }) {
  return <SiteShell activePath="/projects"><section className="project-hero blacksite-hero"><img src={project.image} alt="Kill the Makers project cover" /><div className="project-hero-shade" /><div className="project-hero-copy"><span className="eyebrow">04 / UNITY</span><h1>Kill the Makers</h1><p>A 2D puzzle-platformer about movement, combat, resource management, and mastering environmental interactions.</p></div></section><div className="blacksite-page kill-makers-page">
    ...
    <section className="blacksite-section kill-makers-details"><span className="eyebrow">DESIGN DETAILS</span><div className="blacksite-detail-copy"><h3>Level 1</h3><p>In the first level of the game, I follow Nintendo&apos;s 4-step level design philosophy: Introduction, Development, Twist, and Conclusion.</p><p>First, I placed the camera center-left of the main character to lead the player&apos;s movement. I also designed a small stair to encourage the player to press A and jump onto it.</p><p>The first enemy is the Scientist. Scientists run away when the player approaches, but they cannot harm the player. This creates a safe target for teaching how to defeat enemies and recharge the battery. I used a wall to stop the enemy from running away and make it easier for the player to kill it.</p><p>After players become familiar with the normal jump, I introduce the double jump to make learning feel more rewarding and give them a sense of progress.</p><p>This is the first time acid is introduced. It is relatively safe because the gap only requires one jump to cross, while the double jump gives players enough distance to recover if needed.</p><p>At the end of the level, I teach the dash. The final gap combines basic movement, double jump, and dash as a test of the player&apos;s skills.</p><h3>Level 2</h3><p>In the second level, I introduce a new interaction and a new enemy.</p><p>At the beginning of the level, I teach players how to interact with switches. To communicate the goal, I place a sign on the wall and position the switch directly in front of the gate. In the following section, players gradually master how to use switches.</p><p>This level also introduces a more aggressive enemy. The Guard chases and attacks the player, causing them to lose energy. This reinforces the skill of defeating enemies while adding pressure to the resource-management loop.</p></div></section>
